...
dados <- transform(dados, event = factor(rep("NA", 902297), levels = eventTypes))
for(i in 1:48){
        match <- grep(eventTypes[i], str_trim(toupper(dados$EVTYPE)))
        dados$event[match] <- eventTypes[i]
}
```
  
After matching, **`r sum(is.na(dados$event))`** observations remained not classified and it was assigned **NA** to them.
